...
<results>
	<c:forEach var="row" items="${result.rows}">
		...
		<c:if test="${premium.rowCount != 0}">
			...
			<c:set var="prodId" value="${row.productid}" />
			...
			<result productId="${row.productCat}-${row.productid}">
				...
				<c:choose>
					<c:when test="${type == 'SIN' && children > '0' && row.productId == '29'}">
						<c:set var="newPrice" value="${price.value * 2}" />
						<fmt:setLocale value="en_US"/>
						<premium>${newPrice}</premium>
						<premiumText><fmt:formatNumber value="${newPrice}" type="currency" /></premiumText>
					</c:when>
					<c:when test="${type == 'SIN' && children > '0' && row.productId == '30'}">
						<c:set var="newPrice" value="${price.value * 2}" />
						<fmt:setLocale value="en_US"/>
						<premium>${newPrice}</premium>
						<premiumText><fmt:formatNumber value="${newPrice}" type="currency" /></premiumText>
					...
					<c:when test="${multiTrip != 'Y' && type == 'SIN' && children > '0'}">
						<c:set var="newPrice" value="${price.value +(price.value/2 * children)}" />
						<fmt:setLocale value="en_US"/>
						<premium>${newPrice}</premium>
						<premiumText><fmt:formatNumber value="${newPrice}" type="currency" /></premiumText>
					</c:when>
				...
				<c:forEach var="info" items="${detail.rows}">
					...
					<productInfo propertyId="${info.propertyid}">
						...
						<c:choose>
							<c:when test="${info.propertyid == 'infoDes' && (row.productid == 25 || row.productid == 29)}">
								<text>${info.text} &lt;br&gt; &lt;br&gt; *Note: $250 excess applies to claims under Overseas Medical Expenses and Emergency Repatriation only.</text>
							</c:when>
							<c:otherwise>
								<text>${info.text}</text>
							</c:otherwise>

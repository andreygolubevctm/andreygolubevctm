...
<sql:query var="result">
   SELECT
	...
	WHERE pm.providerId = 5 AND tr.ProductId != '19'
	...
	<c:forEach var="row" items="${result.rows}">
		...
		<c:if test="${row.priceText != null}">
			<result productId="${row.productCat}-${row.productid}">
				...
				<premiumText>${row.priceText}</premiumText>

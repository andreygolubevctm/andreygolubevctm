	...
	<c:forEach var="row" items="${resultRows.rows}">
		<result productId="${row.productCat}-${row.productid}">
			...
			<c:forEach var="info" items="${details.rows}">
						...
						<c:when test="${info.label == 'Additional Expenses'}">
							<c:set var ="desc" value="Additional Accommodation &amp; Travel Expenses" />
						</c:when>
						<c:when test="${info.label == 'Travel Documents, Credit Cards and Travellers Cheq'}">
							<c:set var ="desc" value="Travel Documents &amp; Travellers Cheque" />
							</c:when>
						<c:when test="${info.label == 'Travel Delay Expenses'}">

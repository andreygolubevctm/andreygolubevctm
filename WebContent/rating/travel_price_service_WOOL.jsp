			...
			<c:forEach var="info" items="${details.rows}">
					<c:choose>
						<c:when test="${info.label == 'Overseas Medical'}">
							<c:set var="desc" value="Overseas Medical and Hospital Expenses" />
						</c:when>
						<c:when test="${info.label == 'Cancellation Fees'}">
							<c:set var="desc" value="Cancellation and Amendment Fees" />
						</c:when>
						<c:when test="${info.label == 'Luggage and PE Delay'}">
							<c:set var="desc" value="Delayed Luggage Allowance" />
						</c:when>
						<c:when test="${info.label == 'Rental Vehicle'}">
							<c:set var="desc" value="Rental Car Insurance Excess" />
						</c:when>
						<c:otherwise>

			...
			<c:forEach var="info" items="${details.rows}">
					<c:choose>
						<c:when test="${info.label == 'Overseas Medical'}">
							<c:set var="desc" value="Overseas Medical Expenses" />
						</c:when>
						<c:when test="${info.label == 'Luggage and PE Delay'}">
							<c:set var="desc" value="Baggage Delay (after 12 hours delay)" />
						</c:when>
						<c:when test="${info.label == 'Sudden Dental Expenses'}">
							<c:set var="desc" value="Emergency Dental Treatment" />
						</c:when>
						<c:when test="${info.label == 'Repatriation of remains'}">
							<c:set var="desc" value="Return of Mortal Remains" />
						</c:when>
						<c:when test="${info.label == 'Car Rental Excess Waiver'}">
							<c:set var="desc" value="Rental Vehicle Excess" />
						</c:when>
						<c:when test="${info.label == 'Travel Delay'}">
							<c:set var="desc" value="Delayed Journey to a Special Event" />
						</c:when>
						<c:otherwise>

				...
				<c:forEach var="info" items="${detail.rows}">
					<productInfo propertyId="${info.propertyid}">
						...
						<c:choose>
							...
							<c:when test="${info.label == 'Dental Expenses'}"><desc>Overseas Medical/Hospital/Dental Expenses</desc></c:when>
							...
							<c:when test="${info.label == 'Travel Delay Expenses'}"><desc>Travel Delay - Disruption of Journey</desc></c:when>

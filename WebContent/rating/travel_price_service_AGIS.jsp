...
<c:set var="adults"><x:out select="$travel/request/details/adults" /></c:set>
		...
		<result productId="${row.productCat}-${row.productid}">
			...
			<c:forEach var="info" items="${details.rows}">
				...
				<params>
					...
					<startdate>${reqStartDate}</startdate>
					<enddate>${reqEndDate}</enddate>

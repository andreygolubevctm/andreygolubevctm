...
<c:if test="${multiTrip == 'Y'}">
	<c:set var="priceId">R1-${type}</c:set>
</c:if >
...
<results>
	<c:forEach var="row" items="${resultRows.rows}">
		<result productId="${row.productCat}-${row.productid}">
				...
				</sql:query> <c:forEach var="info" items="${detail.rows}">
		<productInfo propertyId="${info.propertyid}"> <label>${info.label}</label>
						<c:choose>
		...
		</c:choose> <value>${info.value}</value> <text>${info.text}</text> <order>${info.benefitOrder}</order>
					</productInfo>
	</c:forEach> </result>
				</c:forEach>
</results>

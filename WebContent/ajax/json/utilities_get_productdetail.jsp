...
<c:choose>
	<c:when test="${not empty proceedinator and proceedinator > 0}">
		<go:log level="DEBUG" source="utilities_get_productdetail">PROCEEDINATOR PASSED</go:log>
...
<SearchProductDetail xmlns="http://switchwise.com.au/" xmlns:i='http://www.w3.org/2001/XMLSchema-instance'>
	...
	<SearchID>${param.searchId}</SearchID>
		...
		<go:log level="DEBUG" source="utilities_get_productdetail">${resultXml}</go:log>
		<go:log level="DEBUG" source="utilities_get_productdetail">${debugXml}</go:log>
	</c:when>
	<c:otherwise>
		<c:set var="resultXml">

...
<c:choose>
	<c:when test="${not empty proceedinator and proceedinator > 0}">
		<go:log level="DEBUG" source="utilities_get_moveinavailability">PROCEEDINATOR PASSED</go:log>
...
<RetailerSearch xmlns="http://switchwise.com.au/" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">
	...
	<State>${param.state}</State>
		...
		<go:log level="DEBUG" source="utilities_get_moveinavailability">${resultXml}</go:log>
		<go:log level="DEBUG" source="utilities_get_moveinavailability">${debugXml}</go:log>
	</c:when>
	<c:otherwise>
		<c:set var="resultXml">

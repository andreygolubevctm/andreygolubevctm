...
<c:choose>
	<c:when test="${not empty proceedinator and proceedinator > 0}">
		...
		<c:set var="marketing">
			<c:choose>
				<c:when test="${empty data.life.contactDetails.optIn}">N</c:when>
				<c:otherwise>${data.life.contactDetails.optIn}</c:otherwise>
			</c:choose>
		</c:set>
		<c:if test="${not empty data.life.contactDetails.email}">
			<agg:write_email
				brand="CTM"
				vertical="LIFE"
				source="QUOTE"
				emailAddress="${data.life.contactDetails.email}"
				firstName="${data.life.details.primary.firstName}"
				lastName="${data.life.details.primary.lastname}"
				items="marketing=${marketing},okToCall=${data.life.contactDetails.call}" />
		</c:if>
		...
		<go:setData dataVar="data" xpath="life/transactionId" value="${tranId}" />
		<%-- Load the config and send quotes to the aggregator gadget --%>
		<c:import var="config" url="/WEB-INF/aggregator/life/config_results.xml" />
		...
		<c:choose>
			...
			<c:otherwise>
				<c:set var="ignore_me"><core:access_touch quoteType="life" type="R" transaction_id="${data.current.transactionId}" /></c:set>
			</c:otherwise>
		</c:choose>

...
function init_address(name){
		...
		$.getJSON("ajax/html/suburbs.jsp",
			...
			function(resp) {
				if (resp.suburbs && resp.suburbs.length > 0) {
					...
					// AGG-1128 re Meerkat Rewards competition: need to make call when list repopulated
					try{
						$(document).foundation();
					}catch(e){ /* IGNORE */}
				} else {
	...
	if (ok) {
		try{postCodeFld.mask("9999",{placeholder:""});}catch(e){}
	}
	else {
		postCodeFld.numeric();
	}

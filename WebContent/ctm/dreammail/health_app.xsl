								...
								<Value>
									<xsl:choose>
    									<xsl:when test="$env = '_PRO'">
    										<![CDATA[https://secure.comparethemarket.com.au/ctm/health_quote.jsp?action=confirmation&ConfirmationID=]]><xsl:value-of select="$SessionId" /><xsl:if test="$env = '_QAA'"><![CDATA[&sssdmh=dm14.240016]]></xsl:if><xsl:if test="$env = '_PRO'"><![CDATA[&sssdmh=dm14.240054]]></xsl:if><![CDATA[&cid=em:em:health:101911&utm_source=email&utm_medium=email&utm_campaign=email_|_health_|_confirmation&utm_content=review_your_details]]>
    									</xsl:when>
    									<xsl:otherwise><![CDATA[https://qa.secure.comparethemarket.com.au/ctm/health_quote.jsp?action=confirmation&ConfirmationID=]]><xsl:value-of select="$SessionId" /><xsl:if test="$env = '_QAA'"><![CDATA[&sssdmh=dm14.240016]]></xsl:if><xsl:if test="$env = '_PRO'"><![CDATA[&sssdmh=dm14.240054]]></xsl:if><![CDATA[&cid=em:em:health:101911&utm_source=email&utm_medium=email&utm_campaign=email_|_health_|_confirmation&utm_content=review_your_details]]></xsl:otherwise>
									</xsl:choose>	
    							</Value>
							</Variable>
							<Variable>

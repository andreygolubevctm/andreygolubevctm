...
b,0)}:{minHeight:0,height:Math.max(a.height-b,0)}).show();this.uiDialog.is(":data(resizable)")&&this.uiDialog.resizable("option","minHeight",this._minHeight())}});c.extend(c.ui.dialog,{version:"1.8.5",uuid:0,maxZ:0,getTitleId:function(a){a=a.attr("id");if(!a){this.uuid+=1;a=this.uuid}return"ui-dialog-title-"+a},overlay:function(a){this.jQueryel=c.ui.dialog.overlay.create(a)}});c.extend(c.ui.dialog.overlay,{instances:[],oldInstances:[],maxZ:0,events:c.map("focus,mousedown,mouseup,keydown,keypress,click".split(","),
...
(q=d("base")[0])&&l===q.href)){i=f.hash;f.href=i}if(h.test(i))b.panels=b.panels.add(b._sanitizeSelector(i));else if(i&&i!=="#"){d.data(f,"href.tabs",i);d.data(f,"load.tabs",i.replace(/#.*jQuery/,""));i=b._tabId(f);f.href="#"+i;f=d("#"+i);if(!f.length){f=d(c.panelTemplate).attr("id",i).addClass("ui-tabs-panel ui-widget-content ui-corner-bottom").insertAfter(b.panels[g-1]||b.list);f.data("destroy.tabs",true)}b.panels=b.panels.add(f)}else c.disabled.push(g)});if(a){this.element.addClass("ui-tabs ui-widget ui-widget-content ui-corner-all");
...
(function($, undefined) {
	$.widget("ui.binaryoption", {
		...
		_create: function() {
			...
			var yesBox = $('<a>')
				.addClass('binary binary-option-yes')
				.html('Yes')
				.click(function() {
					self.toggleBinary(1);
				});
			...
			var noBox = $('<a>')
				.addClass('binary binary-option-no' )
				.html('No')
				.click(function() {
					self.toggleBinary(0);
				});
			
			this.options.yesBox = yesBox;
			this.options.noBox = noBox;
			
			$(this.element).after(this.options.noBox);
			$(this.element).after(this.options.yesBox);
			
			this.toggleBinary(currentValue);
		},
		...
		toggleBinary: function(val)
		{
			$(this.element).val(parseInt(val));
			
			if (val === 1) {
				$(this.options.yesBox).addClass("binary-on");
				$(this.options.noBox).removeClass("binary-on");
			} else if (val === 0) {
				$(this.options.noBox).addClass("binary-on");
				$(this.options.yesBox).removeClass("binary-on");
			} else {
				return;
			}
			
			// Only trigger the callback on a successful assignment
			var cb = this.options.change;
			if ($.isFunction(cb)) {
				cb(this.element);
			}
		}

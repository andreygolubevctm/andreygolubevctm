...
var k_push_js_revision='$Rev: 13353 $';var k_button1={"Set_Cookie":function(name,value,expires,path,domain,secure)
...
k_push.popup.parentNode.removeChild(k_push.popup);k_push.getSize();document.onmousemove=k_push.mousePos;var maskBG=document.createElement('div');maskBG.setAttribute('id','k_maskBG');document.body.appendChild(maskBG);var popup=document.createElement('div');var use_brand=k_push.branding&&!k_push_vars.no_brand?true:false;popup.setAttribute('id','k_popup');popup.style.width=(k_push_vars.popup_w?k_push_vars.popup_w:k_push.popup_w)+'px';if(!k_push_vars.popup_h)
...
{k_push.closePopup();k_push.counter=0;timeForPopup=(new Date()).getTime()+parseInt(1000*60*2);var main_domain=k_button1.get_main_domain();k_button1.Set_Cookie("push_time_start",timeForPopup,0,"/",main_domain,'');k_button1.Set_Cookie("k_push8",0,0,"/",main_domain,'');},"remind_later":function()
...
{var timeForPopupSession=parseInt(k_button1.Get_Cookie("push_time_start"));}
else
{var seconds_to_delay=k_push_vars.display_after!=undefined?k_push_vars.display_after:k_push.after_time;var timeForPopupSession=(new Date()).getTime()+seconds_to_delay*1000;var main_domain=k_button1.get_main_domain();k_button1.Set_Cookie("push_time_start",timeForPopupSession,0,"/",main_domain,'');}
var seconds_to_delay_page=k_push_vars.display_after_on_page!=undefined?k_push_vars.display_after_on_page:k_push.after_time_on_page;var timeForPopupPage=(new Date()).getTime()+seconds_to_delay_page*1000;timeForPopup=Math.max(timeForPopupSession,timeForPopupPage);if(!k_push_vars.view_percentage)
k_push.n=0;else

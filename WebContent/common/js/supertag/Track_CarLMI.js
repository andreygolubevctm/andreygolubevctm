var Track_CarLMI = new Object();

Track_CarLMI = {

	init: function(){

		Track.init('carlmi','Select');
		...
		Track.resultsShown=function(){
			...
			try {
				...
				var productsArray = Track_CarLMI.getDisplayedProducts();
				superT.trackFeaturedProducts({products:productsArray});

			} catch(err){
				Track_CarLMI.onError("Track.resultsShown: "+err);
			}
		...
		Track.compareClicked= function(){
			try {
				...
				Track_CarLMI.onError("Track.compareClicked: "+err);
			}
	...
	trackBrandSelection:function(brands){
		...
		try {
			...
			Track_CarLMI.onError("Track_CarLMI.trackBrandSelection: "+err);
		}

...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<xsl:choose>
			...
			<xsl:when test="/SOAP-ENV:Envelope/SOAP-ENV:Body/ns2:GetQuotesResponse/ns2:error or /error or ($request/travel/policyType = 'A' and not(//ns3:policy-id = '6'))">
		...
		<results>
			...
			<xsl:for-each select="ns2:quotes">
				...
				<xsl:variable name="uniqueId">
					<xsl:choose>
						<xsl:when test="ns3:policy-id = 5">224</xsl:when> <!-- Ski Plus -->
						<xsl:when test="ns3:policy-id = 6">225</xsl:when> <!-- Annual Ski Plus -->
					</xsl:choose>
				</xsl:variable>

				<xsl:if test="((ns3:policy-id != 6  and $request/travel/policyType = 'S') or (ns3:policy-id = 6 and $request/travel/policyType = 'A'))">
							...
							<xsl:for-each select="ns3:covers">
								<xsl:variable name="nodeName">
									<xsl:choose>
										...
										<xsl:when test="ns3:cover-id = 15">luggage</xsl:when> <!-- Luggage -->
										<xsl:when test="ns3:cover-id = 17">cxdfee</xsl:when> <!-- Cancellation -->

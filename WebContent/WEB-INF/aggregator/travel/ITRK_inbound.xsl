		...
		<results>	
			...
			<xsl:variable name="policyType">
				...
				<xsl:element name="price">
					...
					<quoteUrl>http://www.itrektravelinsurance.com.au/travel/view-quote.html?policyTypeId=<xsl:value-of select="$policyType" />%26destinationCode=<xsl:value-of select="$destinationCode" />%26durationDays=<xsl:value-of select="$durationDays" />%26numberOfAdults=<xsl:value-of select="$adults" />%26numberOfChildren=<xsl:value-of select="$children" />%26adultAges=<xsl:value-of select="$oldest" />%26affID=63</quoteUrl>
				</xsl:element>
			</xsl:for-each>

			...
			<xsl:choose>
				<xsl:when test="policyType = 'S'">
					<xsl:call-template name="getRegionCountryMapping">
						...
						<xsl:with-param name="selectedCountries" select="mappedCountries/VIRG/countries" />
						<xsl:with-param name="selectedRegions" select="mappedCountries/VIRG/regions" />
					</xsl:call-template>
				</xsl:when>

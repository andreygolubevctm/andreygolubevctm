			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<!-- Used Java as a more reliable way of calculating dates
						...
						Did try calls like cal:get(cal:DAY_OF_MONTH) but it always returned 1 which is incorrect and also tried passing DAY_OF_MONTH to the add function with no successDid try calls like cal:get(cal:DAY_OF_MONTH) but it always returned 1 which is incorrect

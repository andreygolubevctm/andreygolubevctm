	...
	<div class="${colWidthValue} short-list-item ${item.getClassString()}">
		...
		<c:choose>
			<c:when test="${item.getType() == 'section'}">			
				<div class="title">
					<h3>${item.getName()}</h3>
					<field_new:switch xpath="${pageSettings.vertical}/benefits/benefitsExtras/${item.getShortlistKey()}" value="Y" className="switch-small ${item.hasShortlistableChildren() ? 'hasChildren' : ''}" />
				</div>				
			</c:when>
			<c:otherwise>

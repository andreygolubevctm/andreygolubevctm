...
<div id="${name}-selection" class="health-dependants">
	...
	<form:fieldset legend="Your Dependants' Details" >
		...
		<div id="${name}_threshold">
			<field:hidden required="true" defaultValue="" xpath="${xpath}/dependantrequired" />
		...
		<div id="dependents_list_options">
			<a href="javascript:void(0);" class="remove-last-dependent tinybtn" title="Remove last dependent"><span>Remove Last Dependent</span></a>
			<a href="javascript:void(0);" class="add-new-dependent tinybtn" title="Add new dependent"><span>Add New Dependent</span></a>
...
var healthDependents = {
	...
	{
		var _dependants = $('#mainform').find('.health_cover_details_dependants').find(':selected').val();
		if( healthCoverDetails.getRebateChoice() == 'N' && _dependants == '') {
...
$.validator.addMethod("limitDependentAgeToUnder25",
	function(value, element) {	
		...
		if( getAge >= healthDependents.maxAge ) {
			...
			$(element).rules("add", { messages: { 'limitDependentAgeToUnder25':'Your child cannot be added to the policy as they are aged ' + healthDependents.maxAge + ' years or older. You can still arrange cover for this dependent by applying for a separate singles policy or please contact us if you require assistance.' } } );
			return false;
		};
		return true;
	}

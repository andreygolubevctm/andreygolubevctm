...
<fieldset id="confirmation-compare-options">
	...
	<div class="options-list clearfix">
		<c:forEach items="${titles}" var="name" varStatus="loop">
			<c:if
				test="${pageSettings.getVerticalCode() ne products[loop.index]}">
				<c:set var="titleParts" value="${fn:split(titles[loop.index], ' ')}" />
				...
				<div class="col-lg-3 col-sm-4 col-xs-6">
					<a href="${pageSettings.getBaseUrl()}${fragments[loop.index]}/"
						title="${titles[loop.index]}">
						<div class="icon icon-${fragments[loop.index]}"></div>${titleParts[0]}<span>${title2}</span>
					</a>
				</div>
			</c:if>
		</c:forEach>

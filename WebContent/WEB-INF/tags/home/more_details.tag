...
<core:js_template id="more-details-template">
	...
	<div class="more-details-template">
		...
		<div id="md-brand">
			<div class="icon"><img src="common/images/logos/home/[#= productId #].png" alt="[#= productDes #]"></div>
		...
		<div id="columns">
			...
			<div class="md-left-column">
				...
				<div id="md-additional-excess-container">
					<div id="md-additional-excess">
						<h5>Additional Excess</h5>
					</div>
					<p>[#= additionalExcess #]</p>
				...
				<div id="md-quote" class="rounded-corners grey-panel">
					...
					<div id="md-price" class="rounded-corners">
						<div class="prices">
							...
							<div class="price annual"><span id="md-annual-price" class="green bold">$[#= price.annual.total #]</span> Annual [#= headline.priceText #]</div>
							<core:clear />
						</div>
						<div class="prices">
						...
						<div class="prices">
							<div class="period HHBExcess">Home Excess:</div>
							<div class="price HHBExcess"><span class="green bold">$[#= HHB.excess.amount #]</span> <span class="excessConditions">refer to Special Conditions and Additional Excess below</span></div>
							<div class="price_separator"></div>
							<div class="period HHCExcess">Contents Excess:</div>
							<div class="price HHCExcess"><span class="green bold">$[#= HHC.excess.amount #]</span> <span class="excessConditions">refer to Special Conditions and Additional Excess below</span></div>
							<core:clear />
						</div>
					</div>
...
<go:script marker="js-head">
	...
	moreDetailsHandler = {
		...
		init : function(prod) {
			...
			if(moreDetailsHandler._product.productAvailable == "Y"){
				moreDetailsHandler.setScrapeType();
		...
		setScrapeType : function () {
			if (moreDetailsHandler._product == false) {
				moreDetailsHandler._product = Results.getResult("productId", moreDetailsHandler._productId);
			}
			if (moreDetailsHandler._product.HHB.description != "" && moreDetailsHandler._product.HHC.description != ""){
				moreDetailsHandler._product.scrapeType = "homeandcontents";
			}
			else if (moreDetailsHandler._product.HHB.description != "" ){
				moreDetailsHandler._product.scrapeType = "home";
			}
			else if (moreDetailsHandler._product.HHC.description != ""){
				moreDetailsHandler._product.scrapeType = "contents";
			}
		},
					...
					success: function(json){
						...
						if(json && json.scrapes && json.scrapes.length > 0){
							$.each(json.scrapes, function(key, scrape){
								if(scrape.html != '' && scrape.cssSelector.indexOf(moreDetailsHandler._product.scrapeType) != -1){ // Need to make sure we have the correct type and that it isnt empty
									$( scrape.cssSelector.replace('-'+moreDetailsHandler._product.scrapeType, '').replace( '#', '#scrape-' ) ).html( scrape.html );
								}
							});
...
<!-- 			Functionality not needed unless offering a similar online discount. Defaulting to off -->
...
<!-- 			} -->
			...
			if (res.headline.terms && res.headline.terms!=''){
				...
				dialogContent.find("#md-special-offer p:last-child").append(" ").append(termsLink);
			}
...
<%-- This is hard coded in the xslt might be added in later --%>
<!-- 			// Add any conditions -->
...
<!-- 			} -->
			...
			if (res.additionalExcess == ""){
				$(dialogContent).find("#md-additional-excess-container").hide();
			}
...
<!-- 			} -->
			...
			if((res.conditions == '' && (res.excess == null || res.excess && res.excess.excess == null))){
		...
		showHideActions : function(){
			...
			// ONLINE
			if (onlineAvailable == "Y" && moreDetailsHandler._product.price && !isNaN(moreDetailsHandler._product.price.annual.total) ){
				actionsCode += 'O';
				$('#md-online').show();
			}

			//OFFLINE
			if (offlineAvailable == "Y" && moreDetailsHandler._product.price && !isNaN(moreDetailsHandler._product.price.annual.total)){
				actionsCode += 'P';
				$('#moreDetailsDialog .offline').show();
			}
			// CALLBACK
			if (callbackAvailable == "Y"){
				actionsCode += 'C';
				$('#moreDetailsDialog #md-offline').show();
			}
		...
		saveCallDirectLeadFeed: function(){
			...
			} else if( $("#home_policyHolder_phone").val() != '' ) {
				var clientTel = $("#home_policyHolder_phone").val();
			}else{
				var clientTel = '';
			}

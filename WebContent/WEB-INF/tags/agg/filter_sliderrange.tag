...
<go:script marker="onready">	
	FilterSet.sliderRange = function(sliderWrapper){	
		var related = $(sliderWrapper).find('input');
		var label = $(sliderWrapper).find('span');
	
		// Value sliders 
		$(sliderWrapper).find('.sliderRange').slider({
			'min': 0,
			'max': 100,
			'range':true,
			'value': [0,100],
			...
			change: function(event, ui) {
				$(related).val(ui.values[0]+"-"+ui.values[1]);
			...
			slide: function(event, ui){
				$(label).html("$"+ui.values[0] + "-$"+ui.values[1]);
			}
		});					
	}
</go:script>
...
<%-- Set up this slider --%>
<go:script marker="onready">
	FilterSet.sliderRange($('#sliderWrapper_${id}'));
		
	<%-- Does the value slider have a step? --%>
	<sql:query var="result">
		SELECT get_property_input_value( '${productCat}', '${propertyId}', 'SLIDERRANGE', 'STEP') as step
	</sql:query>
...
<%-- HTML --%>
<div class="sliderWrapper sliderRange" id="sliderWrapper_${id}">
	<label>${title}</label><span>${description}</span>
	<div class="slider sliderRange ui-slider ui-slider-horizontal ui-widget ui-corner-all" style="background-color:transparent"><a style="left: 0%;" class="ui-slider-handle ui-state-default ui-corner-all" href="#"></a></div>

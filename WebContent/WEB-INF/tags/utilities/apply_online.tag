	...
	<div class="apply-online-template">
		...
		<div id="columns">
			...
			<div class="aol-left-column">
				...
				<div id="aol-header">
					<div id="aol-logo" class="rounded-corners"><img src="common/images/logos/utilities/[#= LogoFileName #]" alt="[#= provider #] Logo" title="[#= provider #] Logo" /></div>
...
<go:script marker="js-head">
	...
	ApplyOnlineDialog = {
		...
		init : function(product, hideApplyBtn){

			ApplyOnlineDialog.setProduct(product);
			ApplyOnlineDialog._hideApplyBtn = hideApplyBtn;
		...
		update : function(){
			...
			product = ApplyOnlineDialog._product;
			...
			var target = product.info.Features;
			...
			if( product.info.Rates.Rate.length == undefined ){
				product.info.Rates.Rate = [product.info.Rates.Rate];
			}
			
			$.each(product.info.Rates.Rate, function(index, value){
				// creating an index
			...
			var termsAndConditions = "<ul><li>" + product.info.TermConditions.TermCondition.join('</li><li>') + "</li></ul>";
			$(dialogContent).find('#aol-terms').append(termsAndConditions);
			
			// PAYMENT OPTIONS
			var paymentOptions = "";

			if (!$.isArray(product.info.PaymentOptions.FeatureGroup.Lines.FeatureLine)) {
				product.info.PaymentOptions.FeatureGroup.Lines.FeatureLine = [product.info.PaymentOptions.FeatureGroup.Lines.FeatureLine];
			}
			$.each(product.info.PaymentOptions.FeatureGroup.Lines.FeatureLine, function(){
				var delimiter;
			...
			if (product.info.Downloads) {
				var documents = "";
				var target = product.info.Downloads;
			...
			Results.negativeValues(product.info.EstimatedSavingText, $("#aol-savings-amount div"), 'extra' );

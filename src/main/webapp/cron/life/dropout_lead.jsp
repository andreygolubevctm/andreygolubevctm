...
<c:set var="logger" value="${go:getLogger('jsp:cron.life.dropout_lead')}" />
		...
		<c:forEach var="result" items="${transactionIds.rows}">
			...
			<c:choose>
				<c:when test="${not empty getTransactionDataError}">
					...
					${logger.error('The CTM life_lead_feed cron job could not get a transaction\'s details result.transaction_id={}', result.transaction_id, getTransactionDataError)}
				</c:when>
				<%-- if there are some results --%>

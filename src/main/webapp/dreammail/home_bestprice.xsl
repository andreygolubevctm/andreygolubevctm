...
<xsl:stylesheet version="1.0"
	...
	<!-- SET THE VARIABLES ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
		<xsl:variable name="gaclientid">
			<xsl:value-of select="home/gaclientid" />
		</xsl:variable>
	...
	<xsl:template name="product" xmlns="http://exacttarget.com/wsdl/partnerAPI">
		...
		<Attributes>
			...
			<Value>
				<xsl:choose>
					<xsl:when test="$emailTokenEnabled = 'true'">
						<xsl:value-of disable-output-escaping="yes" select="concat('&lt;![CDATA[',$baseURL,'email/incoming/gateway.json?gaclientid=',$gaclientid,'&amp;cid=em:cm:hc:500195:hnc_bp&amp;et_rid=172883275&amp;utm_source=hnc_quote_bp&amp;utm_medium=email&amp;utm_campaign=hnc_quote_bp&amp;token=',$currentProduct/loadQuoteToken,']]&gt;')" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of disable-output-escaping="yes" select="concat('&lt;![CDATA[',$baseURL,'email/incoming/gateway.json?gaclientid=',$gaclientid,'&amp;cid=em:cm:hc:500195:hnc_bp&amp;et_rid=172883275&amp;utm_source=hnc_quote_bp&amp;utm_medium=email&amp;utm_campaign=hnc_quote_bp&amp;vertical=home&amp;type=bestprice','&amp;pid=',$productId,'&amp;id=',$tranId,'&amp;email=',$sendToEmail,'&amp;hash=',$hashedEmail,']]&gt;')" />
					</xsl:otherwise>
				</xsl:choose>
			</Value>
		</Attributes>

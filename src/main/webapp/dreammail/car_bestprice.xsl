	...
	<xsl:template name="product" xmlns="http://exacttarget.com/wsdl/partnerAPI">
		...
		<Attributes>
			...
			<Value>
				<xsl:choose>
					<xsl:when test="$emailTokenEnabled = 'true'">
						<xsl:value-of disable-output-escaping="yes" select="concat('&lt;![CDATA[',$baseURL,'email/incoming/gateway.json?cid=em:cm:car:200518:car_bp&amp;et_rid=172883275&amp;utm_source=car_quote_bp&amp;utm_medium=email&amp;utm_campaign=car_quote_bp&amp;token=',$currentProduct/loadQuoteToken,']]&gt;')" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of disable-output-escaping="yes" select="concat('&lt;![CDATA[',$baseURL,'email/incoming/gateway.json?cid=em:cm:car:200518:car_bp&amp;et_rid=172883275&amp;utm_source=car_quote_bp&amp;utm_medium=email&amp;utm_campaign=car_quote_bp&amp;vertical=car&amp;type=bestprice','&amp;pid=',$productId,'&amp;id=',$tranId,'&amp;email=',$sendToEmail,'&amp;hash=',$hashedEmail,']]&gt;')" />
					</xsl:otherwise>
				</xsl:choose>
			</Value>
		</Attributes>

	...
	<xsl:template name="description">
		...
		<xsl:choose>
			...
			<xsl:when test="$productId = 'VIRG-05-26'">
				<xsl:choose>
					<xsl:when test="$productType = 'HHB'">
						<xsl:text>Protect the things you love with a great range of benefits and affordable, flexible cover from Virgin Money.</xsl:text>
					</xsl:when>
					<xsl:when test="$productType = 'HHC'">
						<xsl:text>Protect the things you love with a great range of benefits and affordable, flexible cover from Virgin Money.</xsl:text>
					</xsl:when>
					<xsl:when test="$productType = 'HHZ'">
						<xsl:text>Protect your home and things you love with affordable and flexible Home &amp; Contents cover.</xsl:text>
					</xsl:when>
				</xsl:choose>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="offer">
		...
		<xsl:choose>
			...
			<xsl:when test="$productId = 'VIRG-05-26'">
				<xsl:choose>
					<xsl:when test="$productType = 'HHB'">
						<xsl:text>&lt;b&gt;SPECIAL OFFER:&lt;/b&gt; Your quote for Virgin Home Insurance includes our 10% online discount.</xsl:text>
					</xsl:when>
					<xsl:when test="$productType = 'HHC'">
						<xsl:text>&lt;b&gt;SPECIAL OFFER:&lt;/b&gt; Your quote for Virgin Home Insurance includes our 10% online discount.</xsl:text>
					</xsl:when>
					<xsl:when test="$productType = 'HHZ'">
						<xsl:text>&lt;b&gt;SPECIAL OFFER:&lt;/b&gt; Save 25% on combined Home &amp; Contents Insurance when you buy online from 17 July 2014.</xsl:text>
					</xsl:when>
				</xsl:choose>
			</xsl:when>
		</xsl:choose>

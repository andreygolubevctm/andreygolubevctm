	...
	<xsl:template match="/health">
		...
		<soap:Envelope xmlns:hsl="http://HSL.OMS.Public.API.Service" xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
			<soap:Header xmlns:wsa="http://www.w3.org/2005/08/addressing">
				...
				<wsa:To><xsl:value-of select="$SoapUrl" /></wsa:To>
				...
				<MembershipApplication xmlns="http://www.hambs.com.au/MemberServices/MemberServices.xsd">
					...
					<Persons>
						<Person>
							...
							<FirstName><xsl:value-of select="$primaryFirstname" /></FirstName>
							<Surname><xsl:value-of select="$primarySurname" /></Surname>
							...
							<Properties>
								...
								<xsl:if test="healthCover/rebate='Y'">
									<Property>
										<Name>RebTr</Name>
										<Value><xsl:value-of select="healthCover/income" /></Value>
									</Property>
								</xsl:if>
						...
						<xsl:if test="application/partner/firstname != ''">
							...
							<Person>
								...
								<FirstName><xsl:value-of select="$partnerFirstname" /></FirstName>
								<Surname><xsl:value-of select="$partnerSurname" /></Surname>
								...
								<xsl:if test="$partnerFund != '' and $partnerFund != 'NONE'">
					...
					<Contacts>
						...
						<CorrMethID><xsl:value-of select="$CorrMethID" /></CorrMethID>
						<IsSendEmail><xsl:value-of select="$IsSendEmail" /></IsSendEmail>
						...
						<CommunicationPreferences>
							<xsl:if test="string-length(application/contactPoint) &gt; 0">
								<CommunicationPreference>
									...
									<CommMedium><xsl:value-of select="$CommMedium" /></CommMedium>
									<IsUsed>Y</IsUsed>
								</CommunicationPreference>
								<CommunicationPreference>

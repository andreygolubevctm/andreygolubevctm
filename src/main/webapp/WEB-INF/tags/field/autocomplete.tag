...
<%-- HTML --%>
<input class="ui-autocomplete-input ${className}" id="${name}" name="${name}" value="${value}" ${placeHolderAttribute}<c:if test="${required}"> required
       data-msg-required="Please enter the ${title}" </c:if>>

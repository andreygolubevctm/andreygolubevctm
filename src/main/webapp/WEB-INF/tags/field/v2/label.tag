...
<%@ attribute name="showText" 			required="false" rtexprvalue="true"	 description="Trigger to display help icon as text rather than icon" %>
...
<label <c:if test="${addForAttr eq 'true'}">${forAttr}</c:if> class="${className}">${value}<c:if test='${showHelpIcon eq true}'> <span class="help-icon-with-label hidden-xs"><field_v2:help_icon helpId="${helpId}" showText="${showText}" /></span></c:if></label>

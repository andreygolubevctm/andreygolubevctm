...
<c:choose>
	...
	<c:when test="${style == 'rounded-tile'}">
		<c:set var="classVar" value="btn btn-form-inverse ${classLabel}" />
		<c:set var="className" value='btn-rounded-tile ${className}" data-toggle="radio' />
	</c:when>
	<c:otherwise>
		<c:set var="classVar" value="btn btn-form-inverse ${classLabel}" />
...
<div class="${className}" ${id}>
	<c:forTokens items="${items}" delims="," var="radio" varStatus="status">
		...
		<c:choose>
			<%-- FOR GROUPED STYLE --%>
			<c:when test="${style == 'group' or style == 'group-tile' or style == 'rounded-tile' or style == 'inline' or style == 'vertical'}">

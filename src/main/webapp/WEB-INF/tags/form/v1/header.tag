...
<div id="header" class="clearfix normal-header">
	<div class="inner-header">
		...
		<c:if test="${not empty hasReferenceNo and hasReferenceNo != false}">
			...
			<form_v1:reference_number quoteType="${quoteType}" showReferenceNo="${showReferenceNo}" />
		</c:if>
		<form_v1:contact_panel quoteType="${quoteType}" isCallCentre="${callCentre}" showReducedHoursMessage="${showReducedHoursMessage}" />
		<div id="navigation">

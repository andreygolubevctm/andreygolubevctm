...
<%@ attribute name="submitButton" 	required="false"	description="Step ID that the next button should point to"%>
<%@ attribute name="sticky" 	required="false"	description="Should the fieldset be sticky"%>
...
<c:if test="${ (empty displayFullWidth or displayFullWidth eq false) and hideRightCol ne 'true' }">
	...
	<div class="fieldset-column-side ${sticky ? 'sticky ' : ''} ${stickyTopClass} col-sm-3<c:if test="${not empty sideAbove}"> col-sm-push-9</c:if><c:if test="${not empty sideHidden}"> hidden-xs</c:if>">

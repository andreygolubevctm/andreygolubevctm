...
<%@ attribute name="force3ColSide"		required="false"	rtexprvalue="true"	 	description="Fix for width of right panel in new layout" %>
...
<c:if test="${empty force3ColSide and (empty displayFullWidth or displayFullWidth eq false)}">
...
<c:if test="${not empty force3ColSide}">
	<%--The below isnt formatted nicely for readability (on separate lines) because we want the --%>
	<%--fragment to be empty (no whitespace) if its actually empty so we can hide it--%>

...
if ((allowExternal == 0 && uri.indexOf("secure") != -1) || (allowExternal == 1)) {
	...
	if (typeof(started) == 'undefined' ||  started == 'finished'){
		...
		${id}_wrapper = {
			...
			<go:script marker="head"><core_v1:head quoteType="main" title="${title }" loadjQuery="${loadjQuery}" jqueryuiversion="${jqueryuiversion}" loadjQueryUI="${loadjQueryUI}" jqueryVersion="1.7.2.min" nonQuotePage="${true}"/></go:script>

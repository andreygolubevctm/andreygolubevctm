    ...
    <head>
        ...
        <c:choose>
            <c:when test="${empty skipJSCSS}">
                ...
                <link rel="stylesheet" href="${assetUrl}brand/${pageSettings.getBrandCode()}/css/${pageSettings.getBrandCode()}${pageSettings.getSetting('minifiedFileString')}.css?${revision}"
                      media="all">
                <c:choose>
                    ...
                    <c:when test="${(pageSettings.getVerticalCode() == 'carlmi' or pageSettings.getVerticalCode() == 'homelmi') and pageSettings.getBrandCode() == 'ctm'}">
                        <link rel="stylesheet"
                              href="${assetUrl}brand/${pageSettings.getBrandCode()}/css/lmi.${pageSettings.getBrandCode()}${pageSettings.getSetting('minifiedFileString')}.css?${revision}" media="all">
                        <link rel="stylesheet"
                              href="${assetUrl}brand/${pageSettings.getBrandCode()}/css/${pageSettings.getVerticalCode()}.${pageSettings.getBrandCode()}${pageSettings.getSetting('minifiedFileString')}.css?${revision}"
                              media="all">
                    </c:when>
                    <%-- END HACK FOR LMI --%>
                    <c:otherwise>
                    ...
                    <c:if test="${userAgentSniffer.isSupportedBrowser(pageContext.getRequest(), 'minimumSupportedBrowsers') eq false}">
                        <div class="top-bar">
							...
							<span class="visible-xs">
								Please be aware that any issues you experience on our site could be due to the browser you are using. You may be able to fix these issues by <a
                                    href="http://browsehappy.com/" target="_blank" class="hidden-lg hidden-md hidden-sm">updating your browser.</a>
							</span>
                        </div>
                    </c:if>
                    ...
                    <div class="col-sm-12">
                        ...
                        <nav class="navbar-header header-buttons-logos<c:if test='${not empty header_button_left_class}'> header_button_left</c:if>" role="navigation">
                            ...
                            <c:if test="${not empty exitUrl}">
                                <a id="js-logo-link" href="${fn:toLowerCase(pageSettings.getSetting('exitUrl'))}"
                                   title="${pageSettings.getSetting('windowTitle')}">
                                </c:if>

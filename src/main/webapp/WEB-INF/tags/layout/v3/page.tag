...
<c:set var="urlStyleCodeId">
	<c:if test="${not empty param.brandCode and (fn:toLowerCase(param.brandCode) eq 'ctm' or fn:toLowerCase(param.brandCode) eq 'wfdd' or fn:toLowerCase(param.brandCode) eq 'bddd')}">${fn:toLowerCase(param.brandCode)}</c:if>
</c:set>
...
<c:set var="bodyClass">
	...
	<!-- body content -->
		...
		<%-- JS Libraries --%>
            ...
            <script>window.jQuery && window.jQuery.each || document.write('<script src="${assetUrl}libraries/jquery/js/jquery-3.4.1${pageSettings.getSetting('minifiedFileString')}.js">\x3C/script>');</script>
            <!--<![endif]-->

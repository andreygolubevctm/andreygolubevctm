...
<c:choose>
	<c:when test="${empty param[keyLabel] && (environmentService.getEnvironmentAsString() == 'localhost' || environmentService.getEnvironmentAsString() == 'NXI')}">
		<c:if test="${fn:startsWith(ipAddressHandler.getIPAddress(pageContext.request),'192.168.') or (ipAddressHandler.getIPAddress(pageContext.request),'10.4.')or fn:startsWith(ipAddressHandler.getIPAddress(pageContext.request),'0:0:0:') or ipAddressHandler.getIPAddress(pageContext.request) == '127.0.0.1'}">

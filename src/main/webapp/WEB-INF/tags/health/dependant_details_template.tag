...
<div id="${name}" class="health_dependant_details dependant{{= count }}" data-id="{{= count }}">
	...
	<div class="items">
		...
		<form_new:row fieldXpath="${fieldXpath}" label="Date of Birth" >
			<field_new:person_dob xpath="${fieldXpath}" title="dependant {{= count }}'s date of birth" required="true" ageMin="0" additionalAttributes=" data-rule-limitDependentAgeToUnder25='true' " outputJS="${false}" />
		</form_new:row>

...
<%--&lt;%&ndash; This rule applies to both call center and non call center users &ndash;%&gt;--%>
<%--<c:if test="${not empty callCentre or required}">--%>
	<%--$('#${contactName}').setRequired(true, 'Please enter name');--%>
<%--</c:if>--%>
<%--&lt;%&ndash; These rules are separate to the callCenter one above as they only apply to non simples uers &ndash;%&gt;--%>
<%--<c:if test="${required}">--%>
...
<%--}--%>
<%--});--%>
<%--&lt;%&ndash; COMPETITION END &ndash;%&gt;--%>
<%--</go:script>--%>

<field_v4:contact_number xpath="${xpath}/contactNumber" />

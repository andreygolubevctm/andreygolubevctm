...
<div id="bank-account-fields-group">
    <c:if test="${not empty callCentre and not isClaimAccount}">
        <health_v4_payment:bank_account_details_numbers xpath="${xpath}" className="${rowClass}" useValidator="${true}" />
    </c:if>
    ...
    <c:if test="${empty callCentre or isClaimAccount}">
        <form_v4:row isNestedStyleGroup="true" label="BSB and Account Number">
            <health_v4_payment:bank_account_details_numbers xpath="${xpath}" useValidator="${false}" />
        </form_v4:row>
    </c:if>
</div>
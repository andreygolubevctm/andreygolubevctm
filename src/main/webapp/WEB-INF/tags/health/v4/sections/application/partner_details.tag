    ...
    <div id="partnerextrasfund" class="health-extras_fund">
        <form_v4:fieldset legend="Extras cover">
            ...
            <form_v4:row fieldXpath="${fieldXpath}" label="Who is your partner's current extras cover provider?" id="extrasPartnerFund" className="changes-premium">
                <field_v2:import_select xpath="${fieldXpath}" url="/WEB-INF/option_data/health_funds_condensed.html" title="your partner's extra cover provider" required="true" additionalAttributes=" data-attach='true' " className="combobox" placeHolder="Start typing to search or select from list" requiredErrorMessage="No provider selected." disableErrorContainer="${true}" />
            </form_v4:row>

    ...
    <div id="${name}" class="health_dependant_details dependant{{= obj.dependantId }}" data-id="{{= obj.dependantId }}">
        ...
        <div class="items">
            ...
            <form_v2:row fieldXpath="${fieldXpath}" label="Full-time student" id="${name}_fulltimeGroup"
                ...
                <field_v2:array_radio xpath="${fieldXpath}" required="true" items="Y=Yes,N=No" title="dependant {{= obj.dependantId }}'s full-time status" className="sessioncamexclude"  disableErrorContainer="${false}"
                                      additionalAttributes=" data-rule-validateFulltime='true' " />
            </form_v2:row>
            {{ } }}

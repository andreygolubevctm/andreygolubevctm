...
<core_v1:js_template id="health-dependants-template">
    ...
    <div id="${name}" class="health_dependant_details dependant{{= obj.dependantId }}" data-id="{{= obj.dependantId }}">
        ...
        <div class="items">
            ...
            <form_v3:row label="Name" hideHelpIconCol="true" className="row" isNestedStyleGroup="${true}">
                ...
                <form_v2:row fieldXpath="${fieldXpath}" label="First Name" smRowOverride="3" isNestedField="${true}" hideHelpIconCol="${true}">
                    <field_v2:input xpath="${fieldXpath}" title="dependant {{= obj.dependantId }}'s first name" required="true" className="sessioncamexclude"
                                     additionalAttributes=" data-rule-personName='true' " defaultValue="{{= obj.firstName }}" placeHolder="First name" disableErrorContainer="${true}" />
                </form_v2:row>
                ...
                <form_v2:row fieldXpath="${fieldXpath}" label="Last Name" smRowOverride="3" isNestedField="${true}" hideHelpIconCol="${true}">
                    <field_v2:input xpath="${fieldXpath}" title="dependant {{= obj.dependantId }}'s last name" required="true" className="sessioncamexclude"
                                     additionalAttributes=" data-rule-personName='true'" defaultValue="{{= obj.lastname }}" placeHolder="Last name" disableErrorContainer="${true}" />
                </form_v2:row>

            </form_v3:row>

            <c:set var="fieldXpath" value="${xpath}{{= obj.dependantId }}/dob"/>
            <form_v3:row fieldXpath="${fieldXpath}" label="Date of Birth">
            ...
            <form_v3:row fieldXpath="${fieldXpath}" label="{{= (usesSchoolDropdown ? 'Educational institute this dependant is attending' : 'Name of school your child is attending') }}" id="${name}_schoolGroup"
            ...
            <form_v3:row fieldXpath="${fieldXpath}" label="Student ID Number" id="${name}_schoolIDGroup"
            ...
            <form_v3:row fieldXpath="${fieldXpath}" label="Date Study Commenced" id="${name}_schoolDateGroup"

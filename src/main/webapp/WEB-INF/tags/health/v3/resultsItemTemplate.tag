...
<core_v1:js_template id="feature-template">
    ...
    {{ for(var i = 0; i < featureIterator.length; i++) { var ft = module.getItem(obj, featureIterator[i]); if(ft.doNotRender === true) { continue; } }}
        {{ if(ft.toggle) { var cells = module.getSelectedClinicalCategoriesCells(obj.productId); }}
                ...
                <div class="labelInColumn {{= ft.classStringForInlineLabel }}" {{=ft.labelInColumnTitle }}>
                    <div class="content {{= ft.classStringForInlineLabelCover }}" data-featureId="{{= ft.id }}">
                        <div class="contentInner">
                            <div class="clinical-category-quantity">
                                {{= cells.length }} Clinical {{ cells.length === 1 ? 'category' : 'categories'}} selected
                            </div>
                            <div class="filter-toggle-all selected-benefit-text">
                                <span class="clinical-benefits-text">View details</span> <span class="clinical-benefits-icon icon-angle-down"></span></div>
                        </div>
                    </div>
                </div>
            </div>
            {{ for(var j = 0; j < cells.length; j++) { var benefit = cells[j]; }}
            <div class="cell category collapsed ClinicalCategoryIcon-{{= benefit.class}} clinical-categories-result-card hidden">
                <div class="labelInColumn category collapsed ClinicalCategoryIcon-{{= benefit.class}}">
                    <div class="content">
                        <div class="contentInner">
                            <div style='display:inline-flex'>
                                <span class="health-icon  selected-benefit-icon  ClinicalCategoryIcon-{{= benefit.class}}"></span>
                                <span class="selected-benefit-text">{{= benefit.name }}</span>
                            </div>
                            <div class="benefit-container">
                                <div class="benefit-container-item {{= benefit.cover }}">
        ...
        {{ if(ft.displayItem) { }}
                {{ if(ft.type == 'category') { }}
                    ...
                    {{ if(ft.isHospital || obj.featuresStructureIndexToUse === '3') { }}
                    {{    isSelected = true; }}
                    {{ } }}
                    <div class="labelInColumn {{= ft.classStringForInlineLabel }}" {{=ft.labelInColumnTitle }}>
                        <div class="content {{= ft.classStringForInlineLabelCover }}" data-featureId="{{= ft.id }}">
                            <div class="contentInner" data-analytics="compare BL {{= benefitGroup }}">
                                <div style='display:inline-flex'>
                                    <span class="health-icon {{= isSelected ? ' selected-benefit-icon ' : '' }} {{= ft.iconClass }}"></span>
                                    <span class="{{= isSelected ? 'selected-benefit-text ' : '' }}">{{= ft.safeName }}</span> {{ if(ft.hasChildFeatures) { }}<span class="icon expander"></span>{{ } }}
                                </div>

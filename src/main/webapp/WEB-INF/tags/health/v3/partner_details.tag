...
<form_v2:fieldset legend="Your Partner's Details" id="partnerContainer">
	...
	<div id="${name}">
		...
		<c:set var="fieldXpath" value="health/application/partner/cover/sameProviders" />
			...
			<form_v2:row fieldXpath="${fieldXpath}" label="Partner's Current Health Fund" id="partnerFund" className="changes-premium" renderLabelAsSimplesDialog="true">
				<field_v2:import_select xpath="${fieldXpath}" url="/WEB-INF/option_data/health_funds_condensed.html" title="partner's health fund" required="true" additionalAttributes=" data-attach='true' data-visible='true' " className="combobox" placeHolder="Start typing to search or select from list" disableErrorContainer="${true}" requiredErrorMessage="No provider selected."/>
			</form_v2:row>

			<div id="partnerMemberID" class="membership">
				<c:set var="fieldXpath" value="${xpath}/partner/memberID" />
				<form_v2:row fieldXpath="${fieldXpath}" label="Membership Number" className="partnerMemberID" smRowOverride="3" renderLabelAsSimplesDialog="true">

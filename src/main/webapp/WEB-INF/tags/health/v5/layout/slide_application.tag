...
<layout_v3:slide formId="applicationDetailsForm">

    <layout_v3:slide_content>
        <form_v3:fieldset_columns nextLabel="Proceed to Payment" sideHidden="true">

            <jsp:attribute name="rightColumn">
                <competition:snapshot vertical="health" />
                <health_v4_payment:policySummary showProductDetails="true" />
                <health_v5:price_promise step="apply" dismissible="true" />
            </jsp:attribute>
            ...
            <jsp:body>
                ...
                <c:if test="${isDualPriceActive eq true}">
                    <div class="alert alert-info text-center hidden-if-drop-dead-date-invalid">
                        Remember: Premiums will rise from <span class="pricingDateText"></span>. You must select a cover start date and payment date before <span class="dropDeadDateText"></span> to be eligible for the lower rate.
                    </div>
                </c:if>
                ...
                <health_v4:price_breakdown_templates />

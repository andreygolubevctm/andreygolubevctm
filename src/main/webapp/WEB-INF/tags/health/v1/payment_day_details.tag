...
<div class="${name}-selection">
		...
		<form_v3:row fieldXpath="${fieldXpath}"
			...
			<field_v2:count_select xpath="${fieldXpath}"
						...
						required="true" className="health_payment_day ${name}-day"/>
		</form_v3:row>
		
		<%-- NIB based payment day --%>
		<c:set var="fieldXpath" value="${xpath}/paymentDay" />
		<form_v3:row fieldXpath="${fieldXpath}" noOffsetWhenNoLabel="true"
		...
		<form_v3:row fieldXpath="${fieldXpath}" noOffsetWhenNoLabel="true" className="${name}-details_policyDay-group" renderLabelAsSimplesDialog="true">
			<field_v2:array_select xpath="${fieldXpath}"
						required="true" className="health_payment_day ${name}_details-policyDay" items="=Please choose..." title="your chosen day" />

...
<%@ attribute name="xpath" required="true" rtexprvalue="true" description="field group's xpath"%>
	...
	<form_v2:fieldset legend="Property Features" id="${name}">
		...
		<form_v2:row fieldXpath="${fieldXpath}" label="Is the property fitted with a security alarm?">
			<field_v2:array_radio xpath="${fieldXpath}"
								  title="Is the property fitted with a security alarm?"
								  required="true"
		...
		<form_v2:row fieldXpath="${fieldXpath}" label="Which security features does the property have?" className="securityFeatures" helpId="527" tooltipAttributes="${analyticsAttr}">

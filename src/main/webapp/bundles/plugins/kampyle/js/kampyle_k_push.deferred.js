$(function() {
    var k_push_js_revision = '$Rev: 18687 $';
    var k_button1 = {
    ...
    var k_push = {
        ...
        "mousePos": function (e) {
            if (k_push.popup_open || k_push_vars.popup_manual)
                return;
            k_push.getSize();
            var posy = 0;
            if (!e)e = window.event;
        ...
        "popup_init": function () {
            if (k_push.popup)
                k_push.popup.parentNode.removeChild(k_push.popup);
            k_push.getSize();
            if (window.addEventListener)
                window.addEventListener('mousemove', k_push.mousePos, false); else
                document.attachEvent('onmousemove', k_push.mousePos);
            var maskBG = document.createElement('div');
            maskBG.setAttribute('id', 'k_maskBG');
            document.body.appendChild(maskBG);
            var popup = document.createElement('div');
            var use_brand = k_push.branding && !k_push_vars.no_brand ? true : false;
            popup.setAttribute('id', 'k_popup');
            popup.style.width = (k_push_vars.popup_w ? k_push_vars.popup_w : k_push.popup_w) + 'px';
            if (!k_push_vars.popup_h)
            ...
            k_push.ff_params = (k_push_vars.ff_params) ? k_push_vars.ff_params : false;
            ...
            k_push.remind = k_push_vars.remind ? k_push_vars.remind : 'Remind me later';
            k_push.remind_font_color = (k_push_vars.remind_font_color) ? k_push_vars.remind_font_color : '#3882C3';
            if (k_push.dir == 'ltr') {
            ...
            k_push.images_dir = (k_push_vars.images_dir) ? k_push_vars.images_dir : 'http://cf.kampyle.com/';
            k_push.separator = k_push_vars.popup_separator ? k_push_vars.popup_separator : "#ffffff";
            var landingPage;
            landingPage = "/ff-reg";
            var branding = use_brand ? "<a target='_blank' style='position:relative;bottom:3px' href='http://www.kampyle.com" + landingPage + "?r=push'>Feedback Analytics by</a>" + " &nbsp;<a target='_blank' href='http://www.kampyle.com" + landingPage + "?r=push_logo'><img src='" + k_push.images_dir + "k_logo.gif' /></a>" + "<div id='k_pop_whatisthis'><a href='http://www.kampyle.com" + landingPage + "?r=push_wist&amp;sid=" + k_push_vars.site_code + "' target='_blank' style='position: relative; bottom: -3px;'><img src='" + k_push.images_dir + "k_help.gif' style='vertical-align:bottom;' /></a><a href='http://www.kampyle.com" + landingPage + "?r=push_wist&amp;sid=" + k_push_vars.site_code + "' target='_blank' style='position: relative; bottom: -3px;left:-5px;'>What is this?</a></div>" : "";
            ...
            k_push.footer + "</div>" + "<div id='k_pop_branding'>" + branding + "</div>" + "</div>";
            popup.style.background = (k_push_vars.popup_background) ? k_push_vars.popup_background : '#FFFFFF';
            popup.style.borderColor = k_push.separator;
            k_push.popup = popup;
            document.body.appendChild(popup);
        },
        "handlePostYes": function () {
        },
        "handlePostNo": function () {
        },
        "handlePostRemind": function () {
        },
        "handleYes": function () {
            if (k_push.ff_params && typeof(k_button) != "undefined") {
                ...
                var ff_link = document.getElementById(k_push.ff_link_id);
                if (ff_link) {
                    if (typeof(k_button) == "object" && k_button.ff_link)
                        k_button.ff_link.rel = "&push=1";
                    ff_link.onclick();
                }
            }
            k_push.closePopup();
        ...
        "handleRemind": function () {
            k_push.closePopup();
            k_push.counter = 0;
            var timeForPopup = (new Date()).getTime() + parseInt((1000 * 60 * 2), 10);
            var main_domain = k_button1.get_main_domain();
            k_button1.Set_Cookie("push_time_start", timeForPopup, 0, "/", main_domain, '');
            k_button1.Set_Cookie("k_push8", 0, 0, "/", main_domain, '');
        },
        ...
        "openPopup": function () {
            if (!k_push_vars.popup_manual) {
                ...
                var timeLeft = k_push.initData.timeForPopup - timeNow;
                if (timeLeft > 0)
                    return;
                if (k_button1.Get_Cookie('k_push8') && k_button1.Get_Cookie('k_push8') !== 0 && !k_push_vars.disable_cookie)
                    return;
                if ((k_push.popup_open) || (k_push.counter > 0) || (!document.getElementById(k_push.ff_link_id)))
                    return;
            }
            var main_domain = k_button1.get_main_domain();
            var cookie_expiration_time_no = k_push_vars.cookie_expiration_time_no !== undefined ? k_push_vars.cookie_expiration_time_no : k_push.cookie_expiration_time_no;
            k_button1.Set_Cookie('k_push8', '1', cookie_expiration_time_no, '/', main_domain, '');
            k_push.counter++;
            k_push.popup_open = true;
            var maskBG = document.getElementById('k_maskBG');
            var popup = document.getElementById('k_popup');
            var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
            popup.style.top = (k_push.myHeight / 2) - (k_push.popup_h / 2) + scrollTop + 'px';
            popup.style.left = k_push.myWidth / 2 - k_push.popup_w / 2 + 'px';
            popup.style.display = 'block';
            var inner = document.getElementById("k_popup_inner");
            maskBG.innerHTML = '<div style="width: 100%; height: 100%; opacity: 0.6; filter: alpha(opacity=60); background-color:#111111;"></div>';
            maskBG.style.position = 'absolute';
            maskBG.style.top = '0px';
            maskBG.style.left = '0px';
            maskBG.style.zIndex = '999998';
        ...
        "closePopup": function () {
            var main_domain = k_button1.get_main_domain();
            k_button1.Set_Cookie("push_time_start", "0", 0, "/", main_domain, '');
        ...
        "init": function () {
            if (k_button1.Get_Cookie("push_time_start") > 0) {
                ...
                var seconds_to_delay = k_push_vars.display_after !== undefined ? k_push_vars.display_after : k_push.after_time;
                k_push.initData.timeForPopupSession = (new Date()).getTime() + seconds_to_delay * 1000;
                var main_domain = k_button1.get_main_domain();
                k_button1.Set_Cookie("push_time_start", k_push.initData.timeForPopupSession, 0, "/", main_domain, '');
            }
            var seconds_to_delay_page = k_push_vars.display_after_on_page !== undefined ? k_push_vars.display_after_on_page : k_push.after_time_on_page;
            k_push.initData.timeForPopupPage = (new Date()).getTime() + seconds_to_delay_page * 1000;
            k_push.initData.timeForPopup = Math.max(k_push.initData.timeForPopupSession, k_push.initData.timeForPopupPage);
            ...
            k_push.initData.shouldLoad = false;
            if (k_push_vars.popup_manual) {
    ...
    k_push.init();
    window.k_push_js_revision = k_push_js_revision;
    window.k_push = k_push;
});
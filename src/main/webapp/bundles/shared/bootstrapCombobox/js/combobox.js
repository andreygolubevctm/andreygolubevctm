    ...
    Combobox.prototype = {
        ...
        constructor: Combobox, setup: function () {
            ...
            this.$menu
                ...
                .css({top: pos.top + pos.height, left: pos.left, width:'100%'})
                .show();
            ...
            if(!items.length){
                ...
                items = ["<span class='noResults' style='color:#999999'>No results found for \""+ that.query +"\"</span></br></br><span class='noResultsMsg'> Didn't find the provider you're looking for? <u onmousedown=$(\"input[name="+inputName+"]\").trigger('click');$(\"input[name="+inputName+"]\").trigger('click');>View all providers</u></span>"];
                items = $(items).map(function (i, item) {
                    i = $(that.options.item).attr('data-value', "");
                    i.find('a').html(item);
                    return i[0];
                });
            }else{
            ...
            if (!this.disabled) {
                if (this.$container.hasClass('combobox-selected')) {
                    //this.clearTarget();
                    this.triggerChange();
                    this.lookup(); // custom call for lookup
                    //this.clearElement();
                } else {
                    if (this.shown) {
                        this.hide();
            ...
            if(!$(e.currentTarget).find('.noResults').length){

(function ($, undefined) {
    ...
    function load(fund, callback, performProcess) {
        ...
        if (typeof window['healthFunds_' + fund] === 'undefined' || window['healthFunds_' + fund] === false) {
            ...
            $.ajax({
                url: 'common/js/health/healthFunds_' + fund + '.jsp' + (!_.isEmpty(meerkat.site.urlStyleCodeId) && (_.indexOf(["wfdd", "bddd"], meerkat.site.urlStyleCodeId) >= 0) ? '?brandCode=' + meerkat.site.urlStyleCodeId : ""),
                dataType: 'script',
                data: data,
                async: true,
                timeout: 30000,

    ...
    function setJourneyEngineSteps() {
        ...
        var benefitsStep = {
            ...
            onBeforeEnter: function enterBenefitsStep(event) {
                // configure progress bar
                configureProgressBar(true);
                $('#health_benefits_categorySelectHospital').find('label.icon-health-quickselect-clinicalcat-reduce-tax').toggleClass('hidden', meerkat.modules.healthTiers.getIncome() === '0');
                if (event.isForward) {
                    // Delay 1 sec to make sure we have the data bucket saved in to DB, then filter coupon
                    _.delay(function () {
                        if (meerkat.modules.coupon.doRenderAfterAds() === false) {
    ...
    function getTrackingFieldsObject() {
        try {
            ...
            var response = {
                ...
                cover: {
                    level: null,
                    hospitalScenario: null,
                    extrasScenario: null,
                    hospitalPreferences: null,
                    extrasPreferences: null
                }
            ...
            if (furtherest_step > meerkat.modules.journeyEngine.getStepIndex('details')) {
                ...
                $.extend(response.cover, {
                    level: meerkat.modules.healthBenefitsCoverType.getCoverType(),
                    hospitalScenario: meerkat.modules.healthBenefitsCategorySelectHospital.getSelectedCategory(),
                    extrasScenario: meerkat.modules.healthBenefitsCategorySelectExtras.getSelectedCategory(),
                    hospitalPreferences: meerkat.modules.benefits.getSelectedHospitalBenefits().join(","),
                    extrasPreferences: meerkat.modules.benefits.getSelectedExtrasBenefits().join(",")
                });
            }

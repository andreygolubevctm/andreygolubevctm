...
;(function($, undefined){
	...
	function init(){
		...
		jQuery(document).ready(function($) {
			...
			if (result.hasOwnProperty('data') === false || result.data.status != 'OK' || result.data.product === '') {
				...
				var premium = null;
				var paymentType = meerkat.modules.healthPaymentStep.getPaymentMethodNode(confirmationProduct.paymentType);
				premium = confirmationProduct.paymentTypePremiums[paymentType];
				if (premium !== null && premium[confirmationProduct._selectedFrequency].value && premium[confirmationProduct._selectedFrequency].value > 0)	{
					confirmationProduct.premium = premium;
				}
	...
	function fillTemplate(){
		...
		meerkat.messaging.subscribe(meerkatEvents.healthPriceComponent.INIT, function(selectedProduct){
			...
			meerkat.modules.healthPriceComponent.updateProductSummaryHeader(confirmationProduct, confirmationProduct.frequency, confirmationProduct.startDate, true);
		...
		if (confirmationProduct.paymentTypeAltPremiums) {
			...
			confirmationProduct.altPremium = null;
		}

		if (confirmationProduct.pending === true) {
			//add property to know that product has reached confirmation step but is pending
			confirmationProduct.healthPendingConfirmation = true;
		}

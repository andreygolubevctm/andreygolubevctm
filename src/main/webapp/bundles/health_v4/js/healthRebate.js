...
;(function ($, undefined) {
    ...
    function _eventSubscriptions() {
        $(':input[name="health_situation_healthCvr"], :input[name="health_healthCover_income"], #health_healthCover_dependants').on('change', function updateRebateTiers() {
    ...
    function setRebate(forceRebate) {
        meerkat.modules.healthRates.loadRatesBeforeResultsPage(forceRebate, function (rates) {
            if (!isNaN(rates.rebate) && parseFloat(rates.rebate) > 0) {
                ...
                if ($elements.incomeSelect.filter(':checked').length > 0) {
                    $elements.rebateLegend.html('You are eligible for a ' + rates.rebate + '% rebate. This will be applied as a discount to your premiums.');
                }

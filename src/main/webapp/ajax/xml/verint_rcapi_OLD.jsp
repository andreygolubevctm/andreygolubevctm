		...
		<c:otherwise>
			...
			<c:if test="${not empty getExtensionError or empty authenticatedData.login.user.extension or authenticatedData.login.user.extension eq ''}">
				<c:if test="${not empty getExtensionError}" >
					<go:log level="ERROR" source="verint_rcapi.jsp" error="${getExtensionError}" />
				</c:if>
				<%
...
<c:choose>
	<c:when test="${empty isOperator}">
		<%
		if(!response.isCommitted()) {
			response.sendError(401, "This page is protected." );
		}
		%>
	</c:when>
	<c:when test="${audio != '1' and audio != '0'}">
		<%
		if(!response.isCommitted()) {
			response.sendError(412, "Parameter preconditions failed" );
		}
		%>
	</c:when>
	<c:otherwise>
		<core:verint_rcapi_mute_OLD port="8778" machine="192.168.1.22" audio="${audio}" extension="${authenticatedData.login.user.extension}" />
	</c:otherwise>
</c:choose>
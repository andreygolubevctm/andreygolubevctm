...
<json:array>
	<c:forEach var="row" items="${result.rows}" varStatus="status">
		...
		<json:object>
			...
			<json:property name="unitType" value="${row.unitType}" />
			<json:property name="value" value="${row.unitNo}" />
			<json:property name="highlight" value="${unitType} <b>${fn:substring(row.unitNo,0,searchLen)}</b>${fn:substring(row.unitNo,searchLen, 50)}" escapeXml="false" />
		</json:object>

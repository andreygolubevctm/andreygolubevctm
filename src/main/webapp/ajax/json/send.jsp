...
<c:choose>
	<c:when test="${ignoreEmailSendToUnsubscribed eq true}">
		${logger.info('Email skipped as user not subscribed. {} {}', log:kv('hashedEmail', hashedEmail), log:kv('transactionId', data.current.transactionId))}
	</c:when>
	<c:otherwise>
		...
		<c:choose>
			...
			<c:when test="${param.mode == 'bestprice'}"> <%-- Health's best price email and future ones too --%>
				<c:choose>
					<c:when test="${pageSettings.hasSetting('sendBestPriceMailingName') and pageSettings.hasSetting('sendBestPriceOptInMailingName') and pageSettings.hasSetting('sendBestPriceTmpl')}">
						...
						${logger.debug('[Email] Mode: {} {} {} {} {}', log:kv('mode', param.mode) , log:kv('MailingName', MailingName), log:kv('OptInMailingName', OptInMailingName), log:kv('tmpl', tmpl), log:kv('hashedEmail', hashedEmail))}
					</c:when>
					<c:otherwise>
			...
			<c:otherwise>
				${logger.warn('[Email] No matching mode passed. {} {}', log:kv('mode', param.mode), log:kv('transactionId', data.current.transactionId))}
			</c:otherwise>
		</c:choose>
		...
		<c:choose>
			<c:when test="${empty MailingName}">
				${logger.warn('[Email] Mailing Name empty, no email found to be sent. {} {} {} {} {}', log:kv('transactionId', data.current.transactionId), log:kv('hashedEmail', hashedEmail), log:kv('OptInMailingName', OptInMailingName), log:kv('emailSubscribed', emailSubscribed)}
			</c:when>
			<c:otherwise>

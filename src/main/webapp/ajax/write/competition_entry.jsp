...
<c:set var="logger" value="${go:getLogger('jsp:competition_entry')}" />
...
<c:if test="${not empty param.secret and param.secret == 'F982336B6A298CDBFCECBE719645C'}">
	<c:if test="${not empty param.competitionId}">
		...
		<c:choose>
			...
			<c:when test="${competition_id == '27'}">
				<c:set var="source" value="Life$5000Cash2015" />
			</c:when>
		</c:choose>
	</c:if>
</c:if>
...
<c:if test="${not empty param.secret and param.secret == 'W8C6A452F9823ECBE719DBZFC196C3QB'}">
	<c:if test="${not empty param.competitionId}">
		...
		<c:choose>
			...
			<c:when test="${competition_id == '25'}">
				<c:set var="source" value="Energy$5000CashAug2015" />
			</c:when>
		</c:choose>
	</c:if>
</c:if>

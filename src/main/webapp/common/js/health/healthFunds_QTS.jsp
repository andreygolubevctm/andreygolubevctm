...
var healthFunds_QTS = {
    ...
    set: function(){
        ...
        meerkat.modules.healthFundTimeOffset.onInitialise({
            ...
            renderPaymentDaysCb: healthFunds_QTS.renderPaymentDays
        });
        ...
        healthFunds_QTS.$paymentFrequency.on('change.QTS', function renderPaymentDaysFrequency(){
            healthFunds_QTS.renderPaymentDays();
        });

        healthFunds_QTS.$paymentStartDate.on("changeDate.QTS", function renderPaymentDaysCalendar(e) {
            healthFunds_QTS.renderPaymentDays();
        });
        ...
        $("#health_application_no_email").on("click.QTS",function() {onChangeNoEmailChkBox();});

        <%-- Medicare Fields --%>
        healthFunds_QTS.$medicareFirstname.add(healthFunds_QTS.$medicareLastname).attr('maxlength','24').attr('data-rule-personNameLtd','true').removeAttr('data-rule-personName');
    },
    renderPaymentDays: function(){
        ...
        if (freq == 'fortnightly') {
            ...
            <%-- Redundant code, due to the fact that rules are getting triggered on the payment step for cover start date
                 calendar on the application step, due to the cover start date being moved to the application step. --%>
            <%--healthFunds_QTS.$paymentStartDate.datepicker('setDaysOfWeekDisabled', '');--%>
        }
    ...
    unset: function(){
        ...
        healthFunds_QTS.$paymentFrequency.off('change.QTS');
        healthFunds_QTS.$paymentStartDate.off("changeDate.QTS");
        ...
        <%-- Medicare Fields --%>
        healthFunds_QTS.$medicareFirstname.add(healthFunds_QTS.$medicareLastname).removeAttr('maxlength').removeAttr('data-rule-personNameLtd').attr('data-rule-personName','true');
    }
};
</c:set>

...
g.val(g.getMasked())})};var g={events:function(){a.on("keydown.inputMask",function(){l=g.val()});a.on("keyup.inputMask",g.behaviour);a.on("paste.inputMask",function(){setTimeout(function(){a.keydown().keyup()},100)})},destroyEvents:function(){a.off("keydown.inputMask").off("keyup.inputMask").off("paste.inputMask")},val:function(v){var d="input"===a.get(0).tagName.toLowerCase();return 0<arguments.length?d?a.val(v):a.text(v):d?a.val():a.text()},behaviour:function(a){a=a||window.event;if(-1===c.inArray(a.keyCode||a.which,f.byPassKeys))return g.val(g.getMasked()),
...
b&&"function"==typeof e.onChange)e.onChange(c,f,a,e);if(!0===b&&"function"==typeof e.onKeyPress)e.onKeyPress(c,f,a,e);if("function"===typeof e.onComplete&&c.length===d.length)e.onComplete(c,f,a,e)}};f.remove=function(){g.destroyEvents();g.val(f.getCleanVal()).removeAttr("maxlength")};f.getCleanVal=function(){for(var a=[],c=g.val(),b=0,e=d.length;b<e;b++)f.translation[d.charAt(b)]&&a.push(c.charAt(b));return a.join("")};f.init()};c.fn.inputMask=function(a,d){return this.each(function(){c(this).data("inputMask",
new w(this,a,d))})};c.fn.uninputMask=function(){return this.each(function(){c(this).data("inputMask").remove()})};c("input[data-inputMask]").each(function(){var a=c(this),d={};"true"===a.attr("data-inputMask-reverse")&&(d.reverse=!0);"false"===a.attr("data-inputMask-maxlength")&&(d.maxlength=!1);a.inputMask(a.attr("data-inputMask"),d)})})(window.jQuery||window.Zepto);

...
delete from  ctm.content_control where contentKey = 'footerWebsiteText' and verticalId = @HEALTH and styleCodeId = @CTM_STYLE
 limit 1;
...
select count(*) from ctm.content_control where contentKey in ('footerTextEnd','footerParticipatingSuppliers','footerTextStart')
 ...
 -- expect 0
select count(*) from ctm.content_control where contentKey = 'footerWebsiteText' and verticalId = @HEALTH and styleCodeId = @CTM_STYLE

CREATE OR REPLACE VIEW `simples`.`message_queue_available_hawking` AS
	...
	WHERE
		...
		AND (
			simples.isInAntiHawkingTimeframe(NOW(), msg.state) < 1 /* If customer's timezone is not in hawking time frame, okToCall */
			OR (
				msg.hawkingOptin = 'Y' /* user has to tick the hawkingOptin */
				AND simples.isInAntiHawkingTimeframe(created, msg.state) = 1 /* message has to be created during hawking time frame */
				AND YEARWEEK(created, 1) = YEARWEEK(CURDATE(), 1) /* message has to be created in current week */
			)
		)
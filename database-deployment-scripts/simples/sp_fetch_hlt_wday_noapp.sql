USE `simples`;
DROP procedure IF EXISTS `fetch_hlt_wday_noapp`;

DELIMITER $$
USE `simples`$$
CREATE DEFINER=`server`@`%` PROCEDURE `fetch_hlt_wday_noapp`()
BEGIN
-- -----------------------------
...
-- -----------------------------
-- SET the message_source ID here
-- -----------------------------
...
WHERE
	...
	-- AND header.StartDate = CURRENT_DATE()
	-- AND touchExclude.id IS NULL
...
GROUP BY
 ...
 HAVING
	...
	AND TIMESTAMPDIFF(MINUTE, MAX(TIMESTAMP(touchInclude.date, touchInclude.time)), CURRENT_TIMESTAMP()) BETWEEN 40 AND 1440

...
SELECT
	...
	, (SELECT COUNT(id) FROM simples.message_queue_available WHERE userId IN (0,_userId) AND statusId NOT IN (1, 31, 32, 35 /*PMs are not active leads*/)) AS `Remaining`
	, (SELECT COUNT(id) FROM simples.message WHERE userId IN (0,_userId) AND whenToAction > NOW() AND statusId NOT IN (2, 33, 31, 32, 35 /*PMs and Completed*/)) AS `Future`
;

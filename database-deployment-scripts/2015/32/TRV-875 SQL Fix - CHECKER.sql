...
-- Common variables needed for checkers
...
SET @END_DATE = ("2040-12-31 23:59:59");
 ...
 -- New benefit type - returns one row after update (zero before)
SELECT * FROM `ctm`.`travel_benefit_master` WHERE `benefitName`='CDXHOLIDAYFEE' AND `effectiveStart`=@START_DATE AND `effectiveEnd`=@END_DATE;

-- New/Updated product benefits

...
INSERT into ctm.car_product_features (carProductContentId, code, value, description, effectiveStart, effectiveEnd) SELECT carProductContentId, 'disclaimer', 'The indicative quote is subject to meeting the insurer''s underwriting criteria. Please see below for further details.', '', '2011-03-01','2040-12-31' FROM ctm.car_product cp JOIN ctm.car_product_content cpc ON cp.carProductId = cpc.carProductId WHERE cp.providerId = (SELECT providerId FROM ctm.provider_master WHERE providerCode = 'OZIC') AND cp.code = 'OZIC-05-04';
-- TEST: 0 BEFORE and 1 AFTER
-- SELECT count(*) AS total FROM ctm.car_product_features WHERE code = 'disclaimer' AND carProductContentId = (SELECT cpc.carProductContentId FROM ctm.car_product cp JOIN ctm.car_product_content cpc ON cp.carProductId = cpc.carProductId WHERE cp.providerId = (SELECT providerId FROM ctm.provider_master where providerCode = 'OZIC') AND cp.code = 'OZIC-05-04');
